...
#define SMP_CNT 			1000
#define PID_IN				2.9
#define BLUFF_BODY_IN 0.5

#define CLOCK_FREQUENCY_MHZ 8
#define CODE_VERSION "2.1 07/08/22"   /*   YYYY/MM/DD  */
#define COPYRIGHT "Copyright (c) University of Colorado" 
     
 enum boolean { FALSE, TRUE };           /// \enum boolean  
 enum dmode {QUIET, NORMAL, DEBUG, VERSION, REG_VIEW, MEM_VIEW};       /// \enum dmode 
...
extern double get_Freq(uint16_t* samples);
extern double get_Viscosity(void);
extern double get_Density(void);
extern double get_St(void);
extern double get_Re(void);
extern void get_Vel(void);
extern void get_temp(void);
extern double get_Flow(void);
